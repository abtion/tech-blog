...
CSP is widely deployed but rarely effective. A 2016 Google Research study found that 94.68% of policies attempting to limit script execution are ineffective — and 99.34% of hosts with CSP use policies that offer no XSS benefit at all.[^csp-stats] A decade on, a June 2026 crawl of the Tranco Top 1 Million sites shows the same pattern: of the 170,057 sites now sending a CSP header, **46.8% still contain `'unsafe-inline'`** and 41.9% contain `'unsafe-eval'` — and only 1.6% use `'strict-dynamic'`.[^csp-2026] The underlying problem — third-party tools, legacy scripts, and CMS platforms defaulting to `'unsafe-inline'` — has not gone away.
